...
def main():
    ...
    while True:
        ...
        for i in range(2):
            ...
            again = eg.buttonbox(f"You rolled: {dice} \n"
        ...
        eg.msgbox(f"Your final roll is: {dice} \n"
                  f"{detect_result(dice)}", "Result!")

        # 유저가 3개의 턴을 전부 행하면, 다시 플레이하고 싶은지 물어본다.
        restart = eg.buttonbox("You've finished the game. Do you want to play again?", "Try?", choices=["Yes", "No"])

        # 만약 아니라고 대답한다면, 종료 인사를 표시한 뒤 while loop을 break한다.
        if restart == "No":
            eg.msgbox("Thank you for playing Yahtzee lite!", "Thank you!")
            break
